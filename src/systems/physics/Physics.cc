...
void PhysicsPrivate::UpdateSim(EntityComponentManager &_ecm) const
{
  ...
  // TODO(anyone): this should be latched and there is no need to update
  // unless we want to use a world model where we update the field value
  // based on a World Magnetic Model like WMM2015
  _ecm.Each<components::MagneticField>(
      [&](const Entity & /*_entity*/,
          components::MagneticField *_magneticField)->bool
      {
        *_magneticField = components::MagneticField(this->magneticField);
        return true;
      });
}
